...
namespace crest {
  ...
  public:
    ...
    template <typename T> void set(key k, T value) { store_.put(name(k), value); }

    static const char *name(key k) { return get_spec<0>(k); }

    static const char *description(key k) { return get_spec<1>(k); }
